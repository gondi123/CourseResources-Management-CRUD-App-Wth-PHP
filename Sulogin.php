  ...
  <body >
	...
	<?php
		    ...
		    if ($_SERVER["REQUEST_METHOD"] == "POST")
		   {
			...
			if(empty($name))
			{
			$nameErr="<font color='red'>rolll number  is required</font>";
            }
			else
			{
			 ...
			 if(preg_match("/^[a-zA-Z ]*$/",$name))
			 {
				...
				die("<h2> 404 Error please provide proper roll number</h2>");
			 }
		    }
            if(empty($_POST["input2"]))
			  ...
			  $query="select * from stsignup where name='$name' and password='$pass'";
			  $result=mysql_query($query);
			  $count=mysql_num_rows($result);
			  if($count<=0){
                   die("<h2>404 Error invalid</h2>");
              }else{
				  echo "<script>window.location.assign('download.php')</script>";
			  }
	 ...
	 <h1> Login</h1></center>
	  <div>
	    <form action="Sulogin.php" method="POST">
         
<fieldset><legend><font color="blue">Login</font></legend>
		 
			rollnumber <input type='text'name="input1"><br/><br/>
